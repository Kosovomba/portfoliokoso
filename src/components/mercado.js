...
export default function Mercado({clase, raza, mercado, setMercado, personaje, setPersonaje}){
    let n = (personaje['apt2+'].filter((a)=> a.nombre === 'Luchador versátil: ').length !== 0 || personaje.apt1[2][0] === 'Combate con dos armas: 1xturno, cuando usa 1d6 de acción para atacar, lanza otro d6 extra de ataque. El explorador puede comprar una segunda arma para obtener daño por arma para su ataque extra (el arma extra sólo sumará daño a ataques extra de combate con dos armas).')?2:1
    console.log(personaje.CDP)

    let handleDisabled = {
        Armadura: personaje.equipamiento[1][0] === 0 && (clase !== 'Mago' && clase !== 'Monje' && clase !== 'Hechicero' && clase !== 'Oráculo' && clase !== 'Ninja')?false:true,
        'Armadura pesada': personaje.equipamiento[2][0] === 0 && personaje['apt2+'].filter((a)=> a.nombre === 'Armadura pesada: ').length !== 0?false:true,
        'Arma cuerpo a cuerpo': personaje.equipamiento[3][0] < n?false:true,
        'Arma de ninja': personaje.equipamiento[10][0] === 0 && (personaje.CDP.nombre && personaje.CDP.nombre === 'Ninja ocre: ')?false:true
    }
    ...
    return (
        ...
        <div style={{overflowY:'scroll', overflowX:'hidden'}} >            
            ...
            <p className={styles.p}>Armadura: Otorga +1 a RD, por armadura equipada.<button disabled={handleDisabled.Armadura} onClick={handleComprar} value='1' className={styles.b}>Comprar (2 PP)</button></p>
            <p className={styles.p}>Armadura pesada (sólo guerrero, caballero o paladín, cuando tengan la aptitud correspondiente): Otorga +2 a RD, por armadura equipada.<button disabled={handleDisabled['Armadura pesada']} onClick={handleComprar} value='2' className={styles.b}>Comprar (3 PP)</button></p>
            <p className={styles.p}>Arma cuerpo a cuerpo: Otorga +1 al daño de ataques cuerpo a cuerpo, por arma equipada.<button disabled={handleDisabled['Arma cuerpo a cuerpo']} onClick={handleComprar} value='3' className={styles.b}>Comprar (3 PP)</button></p>
            ...
            <p className={styles.p}>Arma de ninja: Otorga +1 al daño por arma equipada, a la aptitud del Ninja ocre.<button disabled={handleDisabled['Arma de ninja']} onClick={handleComprar} value='10' className={styles.b}>Comprar (1 PP)</button></p>            
        </div>
        </div>
    )
}