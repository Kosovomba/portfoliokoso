export default function conjuroInicial({personaje, setPersonaje, raza, clase, conjurosIniciales}) {    
    ...
    return (
        <div>
            ...
            {conjurosInicialesFiltrados.map((c)=> <button style={{maxWidth: '400px'}} onClick={handleButton} key={`${c.nombre}${c.aptitud}`} value={`${c.nombre}${c.aptitud}`}>{`${c.nombre}${c.aptitud}`}</button>)}</div>:null}            
