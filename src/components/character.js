    ...
    return (
        <div>
            ...
            <div style={{display:'flex', flexFlow:'wrap', minWidth: '95%', justifyContent: 'center'}}>
                <button onClick={handlePestaña} value={'opciones'} disabled={pestaña === 'opciones'?true:false} style={{fontWeight: 630, height:'50px', width:'127px'}}>Opciones</button>
                <button onClick={handlePestaña} value={'racialesEIniciales'} disabled={pestaña === 'racialesEIniciales'?true:false} style={{fontWeight: 630, height:'50px', width:'127px', border:(personaje.clase === 'Explorador' && personaje.apt1[2].length === 0) || (claseStats['conjuros iniciales'] && personaje.apt1[2].length + (clase === 'Mago'?0:1) < 2) ?'solid 2px red':null}}>Aptitudes raciales e iniciales{(personaje.clase === 'Explorador' && personaje.apt1[2].length === 0) || (claseStats['conjuros iniciales'] && personaje.apt1[2].length + (clase === 'Mago'?0:1) < 2)?<FaArrowDown style={{color:'red', fontSize: 22, position:'absolute', pointerEvents:'none'}}/>:null}</button>
                <button onClick={handlePestaña} value={'2oMayor'} disabled={pestaña === '2oMayor'?true:false} style={{fontWeight: 630, height:'50px', width:'127px', border:personaje['apt2+'].length + 1 < personaje.nivel?'solid 2px red':null}}>Aptitudes de nivel 2 o mayor{personaje['apt2+'].length + 1 < personaje.nivel?<MdOutlinePlusOne style={{color:'red', fontSize: 25, position:'absolute', pointerEvents:'none'}}/>:null}</button>
                <button onClick={handlePestaña} value={'cdp'} disabled={pestaña === 'cdp'?true:false} style={{fontWeight: 630, height:'50px', width:'127px'}}>Clase de prestigio</button>
                <button onClick={handlePestaña} value={'estadísticas'} disabled={pestaña === 'estadísticas'?true:false} style={{fontWeight: 630, height:'50px', width:'127px'}}>Estadísticas/Uso del personaje</button>
                <button onClick={handlePestaña} value={'equipamiento'} disabled={pestaña === 'equipamiento'?true:false} style={{fontWeight: 630, height:'50px', width:'127px'}}>Equipamiento</button>
            </div>
